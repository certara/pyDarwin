...
class SQLiteModelCache(ModelCache):
    ...
    def store_model_run(self, run: ModelRun):
        ...
        values = (
            ...
            r.post_run_python_penalty if not options.isMOGA else None,
            r.post_run_python_text if not options.isMOGA else None,
            r.post_run_r_penalty if not options.isMOGA else None,
            r.post_run_r_text if not options.isMOGA else None,
            r.errors,
            r.messages,
            run.file_stem,
            run.run_dir,
